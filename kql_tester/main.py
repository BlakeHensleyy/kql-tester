...
# Test type descriptions for help text
test_descriptions = """
...
def get_azure_credential():
    """
    Get Azure credential using multiple authentication methods in order of preference:
    ...
    # Method 2: Try DefaultAzureCredential (handles CLI, Managed Identity, etc.)
    try:
        ...
        next(sub_client.subscriptions.list())  # This will fail if credential doesn't work
        return credential
    except Exception as e:
        print(f"DefaultAzureCredential failed: {e}")
    
    # Method 3: Interactive browser login as last resort
    print("Falling back to interactive browser authentication")
    from azure.identity import InteractiveBrowserCredential
    return InteractiveBrowserCredential()

def get_workspace_id():
    """Get Log Analytics workspace ID from environment or prompt user"""
    workspace_id = os.getenv('LOGS_WORKSPACE_ID') or os.getenv('LOG_ANALYTICS_WORKSPACE_ID')
    
    if not workspace_id:
        print("Log Analytics Workspace ID not found in environment variables.")
        print("Please set LOGS_WORKSPACE_ID or LOG_ANALYTICS_WORKSPACE_ID")
        print("Example: export LOGS_WORKSPACE_ID='12345678-1234-1234-1234-123456789012'")
        sys.exit(1)
    
    return workspace_id

def main():
    """Main function for KQL tester"""
    ...
    test_result = "None"
    IncludeDataRows = 10     # Limits the data to 10 results as more than this results in an inefficient YAML file.
    ...
    # Initialize the Azure Monitor Logs client with flexible authentication
    try:
        ...
        print("Please ensure you're logged in with 'az login' or have set the required environment variables.")
        sys.exit(1)
    ...
    try:
        ...
        if include_data:
            ...
            if data: 
                ...
                for table in data:
                    truncated_rows = table.rows[:IncludeDataRows]
                    formatted_data = []
